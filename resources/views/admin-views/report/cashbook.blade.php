...
@section('title', \App\CPU\translate('CashBook Report'))
    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                ...
                {{\App\CPU\translate('CashBook Report')}}
            </h2>
        </div>
        <!-- End Page Title -->
        ...
        <div class="card">
            <div class="card-header border-0">
                <div class="d-flex flex-wrap w-100 gap-3 align-items-center">
                    <h4 class="mb-0 mr-auto">
                        {{\App\CPU\translate('Opening Balance')}}
                        <span class="badge badge-soft-dark radius-50 fz-14">{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($openingBalance)) }}</span>
                    </h4>
            ...
            <div class="table-responsive">
                <table id="datatable"
                    ...
                    <tr>
                        ...
                        <th>{{\App\CPU\translate('Closing Bal')}}</th>
                    ...
                    <tbody>
                        @foreach ($transactions as $transaction)
                            <tr>
                                ...
                                @if($transaction['type'] == 'Order' || $transaction['type'] == 'Receipt')
                                    <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($transaction['amount'])) }}</td>
                                @else
                                <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency(0.00)) }}</td>
                                @endif
                                @if($transaction['type'] == 'Expenses' || $transaction['type'] == 'Cash Deposit')
                                <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($transaction['amount'])) }}</td>
                                @else
                                <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency(0.00)) }}</td>
                                @endif
                                
                                <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($transaction['running_balance'])) }}</td>
                            </tr>
                        @endforeach
            ...
            <h4 class="mb-0" style="display: flex;justify-content: right;margin-right: 100px;margin-bottom: 10px;">
                {{\App\CPU\translate('Closing Balance')}}
                <span class="badge badge-soft-dark radius-50 fz-14">{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($closingBalance)) }}</span>
            </h4>
        </div>
